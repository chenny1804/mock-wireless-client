#-*-coding:utf-8-*-
import hmac,hashlib,binascii,threading
...
class test_wlan_frame():
	def __init__(self,radioband,clientmac,interface,ssid=None,apmac=None,crypto=0):
	...
	def __assoc_init(self):
		if self.radioband ==1 and self.crypto == 0:
			assoc=rdpcap("assocreq24.pcap")[0]
		elif self.radioband ==1 and self.crypto == 1:
			assoc=rdpcap("assocreq24-wpa-AES.pcap")[0]
		elif self.radioband ==1 and self.crypto == 2:
			assoc=rdpcap("assocreq24-wpa2-AES.pcap")[0]
		elif self.radioband==0:
...
def handshake(pkt):
	...
	if b2a_hex(pkt[EAPOL].load[1:3]) == '008a' and pkt.addr1==STA_MAC and pkt.addr2==AP_MAC:
		...
		print "sta_keyinfo:",b2a_hex(PTK[32:48])
		mic="0"*32
		eapol_2[0].addr1=eapol_2[0].addr3=AP_MAC
		eapol_2[0].addr2=STA_MAC
		print eapol_2[0].addr2
		...
		#rsppkt,unans=srp(eapol_2[0],iface=INTERFACE,timeout=10)
		sendp(eapol_2[0],inter=0.1,iface=INTERFACE,count=1)
		#sys.exit(0)
		'''
		mic='0'*32
		eapol_4[0].addr1=eapol_4[0].addr3=AP_MAC
		eapol_4[0].addr2=STA_MAC
		...
		sendp(eapol_4[0],iface=INTERFACE,count=5)
		'''
		send_eapol4(pkt)
	else:
		pkt.show()
	#if b2a_hex(pkt[EAPOL].load[1:3]) == '13ca':
def send_eapol4(pkt):
...
def get_ptk(ANonce):
	...
	print "PMK",psk
	return ptk
def get_sniff(function):
	if function == "send_eapol4":
		sniff(count=1,iface="mon0",prn=send_eapol4,lfilter=lambda x: x.haslayer(EAPOL))
	elif function == "handshake":
		sniff(count=1,iface="mon0",prn=handshake,lfilter=lambda x: x.haslayer(EAPOL))
def test_wpa2():
	t1=threading.Thread(target=get_sniff,args=("handshake",))
	#t2=threading.Thread(target=get_sniff,args=("send_eapol4",))
	t1.start()
	#t2.start()
	Test1=test_wlan_frame(BAND,STA_MAC,INTERFACE,SSID,AP_MAC,CRYPTO)
	Test1.test_suit_normal()
	#Test1.send_assoc()
if __name__== "__main__":
	BAND=1												#channel band value: 0 is 5G  and 1 is 2.4G 
	CRYPTO=2#0:None 1:wpa 2:wpa2
	INTERFACE="mon0"                        #interface of send frame
	SSID="wpatest"
	#SSID="TP-LINK_DD9A_2.4G"
	PASSPHRASE="1234567890"					#AP's SSID
	#AP_MAC="00:00:1c:00:85:23"		#AP's MAC
	AP_MAC="08:10:0c:00:0c:24"
	#AP_MAC="dc:fe:18:14:dd:9a"
	STA_MAC="50:01:d9:a6:85:44"
	CHANNEL=13
